/**
 * TP03Q011 - Quicksort com Lista Dupla em java
...
class Lista {
    ...
    private int tamanho, comparacoes, movimentacoes;

    /**
     * Construtor padrão que inicializa a lista com uma célula vazia.
     */
    public Lista() {
        primeiro = new Celula();
        ultimo = primeiro;
    ...
    /**
     * Obtém a célula em uma posição específica da lista.
     *
     * @param pos A posição desejada na lista.
     * @return A célula na posição especificada.
     */
    public Celula getElement(int pos) {
    ...
    /**
     * Troca os elementos de duas células.
     *
     * @param a A primeira célula.
     * @param b A segunda célula.
     */
    ...
    /**
     * Ordena a lista utilizando o algoritmo quicksort.
     */
    ...
    /**
     * Método auxiliar para realizar o quicksort em uma parte específica da lista.
     *
     * @param esquerda Índice do início da parte a ser ordenada.
     * @param direita Índice do final da parte a ser ordenada.
     */
    public void quicksort(int esquerda, int direita) {
        ...
        Celula l = getElement(esquerda);
        Celula r = getElement(direita);
        Jogador h = getElement(pivo).elemento;

        // Parte principal do algoritmo quicksort
        while (i <= j) {
            while (l.elemento.getEstadoNascimento().compareTo(h.getEstadoNascimento()) < 0
                    || (l.elemento.getEstadoNascimento().compareTo(h.getEstadoNascimento()) == 0
                            && l.elemento.getNome().compareTo(h.getNome()) < 0)) {
                comparacoes++;
                l = l.prox;
                i++;
            }

            while (r.elemento.getEstadoNascimento().compareTo(h.getEstadoNascimento()) > 0
                    || (r.elemento.getEstadoNascimento().compareTo(h.getEstadoNascimento()) == 0
                            && r.elemento.getNome().compareTo(h.getNome()) > 0)) {
                comparacoes++;
                r = r.ant;
                j--;
            }
    ...
    public void mostrar() {
        ...
        for (int i = 0; i < tamanho; i++, tmp = tmp.prox) {
            System.out.println("[" + tmp.elemento.imprimir() + "]");
        }
    }
}
...
public class Quicksort {
    ...
    public static void main(String[] args) throws Exception{
        //arquivo de log
        FileWriter arquivo = new FileWriter ("740136_quicksort.txt");
        BufferedWriter quicksort = new BufferedWriter(arquivo);
        quicksort.write("740136" + "\t");
        ...
        quicksort.write((tempoFinal - tempoInicial)+ "ms" + "\t");
    
        //escrever número de comparações e movimentações da ordernação
        quicksort.write("" + lista.getComparacoes() + "comparações\t");
        quicksort.write("" + lista.getMovimentacoes() + "movimentações");
