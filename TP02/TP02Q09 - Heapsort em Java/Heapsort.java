/**
 * TP02Q09 - Ordenacao por heapsort em java
...
class Lista {
    ...
    //alterar construir e reconstruir
    private void construir(int tamanho) {
        for(int i = tamanho; i > 1; i/=2) {
            comparacoes++;
        if (arrayJogadores[i].getAltura() > arrayJogadores[i / 2].getAltura() || 
            (arrayJogadores[i].getAltura() == arrayJogadores[i / 2].getAltura() &&
             arrayJogadores[i].getNome().compareTo(arrayJogadores[i / 2].getNome()) > 0)) {
            swap(i, i / 2);
        }
        }
    }

    private boolean hasFilho(int i, int tamanho) {
        return ( i <= (tamanho/2));
    }

    private int getMaiorFilho(int i, int tamanho) {
        int filho;
        comparacoes++;
        if (2 * i == tamanho || arrayJogadores[2 * i].getAltura() > arrayJogadores[2 * i + 1].getAltura() ||
            (arrayJogadores[2 * i].getAltura() == arrayJogadores[2 * i + 1].getAltura() &&
            arrayJogadores[2 * i].getNome().compareTo(arrayJogadores[2 * i + 1].getNome()) > 0)) {
            filho = 2 * i;
        }
        else {
            filho = 2*i + 1;
    ...
    private void reconstruir(int tamanho) {
        int i = 1;
        while(hasFilho(i, tamanho) == true) {
            int filho = getMaiorFilho(i, tamanho);
            comparacoes++;
            if (arrayJogadores[i].getAltura() < arrayJogadores[filho].getAltura() ||
                (arrayJogadores[i].getAltura() == arrayJogadores[filho].getAltura() &&
                arrayJogadores[i].getNome().compareTo(arrayJogadores[filho].getNome()) < 0)) {
                swap(i, filho);
                i = filho;
            }
            else {
    ...
    /** Algoritmo para ordernar o array de jogadores por inserção */
    public void heapsort() {
        //data
        Jogador[] tmp = new Jogador[n + 1];

        for ( int i = 0; i < n; i++ ) {
            tmp[i+1] = arrayJogadores[i];
        }
...
public class Heapsort {
    ...
    public static void main(String[] args) throws Exception{
        //arquivo de log
        FileWriter arquivo = new FileWriter ("740136_heapsort.txt");
        BufferedWriter arquivoHeapsort = new BufferedWriter(arquivo);
        arquivoHeapsort.write("740136" + "\t");
        ...
        arquivoHeapsort.write((tempoFinal - tempoInicial)+ "ms" + "\t");
    
        //escrever número de comparações da pesquisa
        arquivoHeapsort.write("" + lista.getComparacoes() + "comparações\t");
        arquivoHeapsort.write("" + lista.getMovimentacoes() + "movimentações");

        //close file
        arquivoHeapsort.close();
        arquivo.close();
    }
}
