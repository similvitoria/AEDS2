/**
 * TP02Q15 - Ordenacao por selecao parcial em java
...
class Lista {
    ...
    /** Algoritmo para ordernar o array de jogadores por seleção */
    public void selecao(int k) {
        //dados
        int menor = 0;
        ...
        for(int i = 0; i < k; i++) {
            ...
            for(int j = (i + 1); j < n; j++) {
                comparacoes++;
                if(arrayJogadores[j].getNome().compareTo(arrayJogadores[menor].getNome()) < 0) {
                    menor = j;
                }
            }
            swap(i, menor);
...
public class SelecaoParcial {
    ...
    public static void main(String[] args) throws Exception{
        //arquivo de log
        FileWriter arquivo = new FileWriter ("740136_selecao.txt");
        BufferedWriter selecao = new BufferedWriter(arquivo);
        selecao.write("740136" + "\t");
        ...
        selecao.write((tempoFinal - tempoInicial)+ "ms" + "\t");
    
        //escrever número de comparações e movimentações da ordernação
        selecao.write("" + lista.getComparacoes() + "comparações\t");
        selecao.write("" + lista.getMovimentacoes() + "movimentações");
