/**
 * TP02Q03 - Pesquisa sequencial em java
...
class Lista {
    ...
    /**
     * @param nomeJogador O nome do jogador a ser pesquisado.
     * @return "SIM" se o jogador for encontrado na lista, "NAO" caso contrário.
     */
    public String pesquisaSequencial(String nomeJogador){
        //dados
        String resposta = "NAO";

        for (int i = 0; i < n; i++) {
            comparacoes++;
            if (arrayJogadores[i].getNome().contains(nomeJogador)) {
                resposta = "SIM";
                i = n;
            }
        }
...
public class PesquisaSequencial {
    ...
    public static void main(String[] args) throws Exception{
        //arquivo de log
        FileWriter arquivo = new FileWriter ("740136_pesquisaSequencial.txt");
        BufferedWriter arquivoSequencial = new BufferedWriter(arquivo);
        arquivoSequencial.write("740136" + "\t");
        ...
        //tempo antes de executar a pesquisa sequencial
        int tempoInicial = (int)System.currentTimeMillis();

        while(!(isFIM(entrada = MyIO.readLine()))) {
            System.out.println(lista.pesquisaSequencial(entrada));
        }

        //tempo depois de executar a pesquisa sequencial
        int tempoFinal = (int)System.currentTimeMillis();
        arquivoSequencial.write((tempoFinal - tempoInicial)+ "ms" + "\t");
    
        //escrever número de comparações da pesquisa
        arquivoSequencial.write("" + lista.getComparacoes() + "comparações");

        //close file
        arquivoSequencial.close();
        arquivo.close();
    }
}