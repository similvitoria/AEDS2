/**
 * TP02Q07 - Ordenacao por inserção em java
...
class Lista {
    ...
    public void insercao() {
        ...
        for(int i = 1; i < n; i++) {
            tmp = arrayJogadores[i];
            j = i - 1;
            while((j >= 0) && (arrayJogadores[j].getAnoNascimento() > tmp.getAnoNascimento() ||
            arrayJogadores[j].getAnoNascimento() == tmp.getAnoNascimento() &&
            arrayJogadores[j].getNome().compareTo(tmp.getNome()) > 0)) {
                arrayJogadores[j + 1] = arrayJogadores[j];
                movimentacoes++;
                comparacoes++;
                j--;
            }
            arrayJogadores[j + 1] = tmp;
            movimentacoes++;
        }
    }
...
public class Insercao {
    ...
    public static void main(String[] args) throws Exception{
        //arquivo de log
        FileWriter arquivo = new FileWriter ("740136_insercao.txt");
        BufferedWriter arquivoInsercao = new BufferedWriter(arquivo);
        arquivoInsercao.write("740136" + "\t");
        ...
        arquivoInsercao.write((tempoFinal - tempoInicial)+ "ms" + "\t");
    
        //escrever número de comparações da pesquisa
        arquivoInsercao.write("" + lista.getComparacoes() + "comparações\t");
        arquivoInsercao.write("" + lista.getMovimentacoes() + "movimentações");

        //close file
        arquivoInsercao.close();
        arquivo.close();
    }
}