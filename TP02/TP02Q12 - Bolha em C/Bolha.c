...
void bolha(Lista *lista) {
    for (int i = lista->n - 1; i > 0; i--) {
        for (int j = 0; j < i; j++) {
            lista->comparacoes++;
            if (lista->array[j].anoNascimento > lista->array[j + 1].anoNascimento) {
                swap(j, j + 1, lista);
            } else if (lista->array[j].anoNascimento == lista->array[j + 1].anoNascimento) {
                lista->comparacoes++;
                if (strcmp(lista->array[j].nome, lista->array[j + 1].nome) > 0) {
                    swap(j, j + 1, lista);
                }
            }
        }
