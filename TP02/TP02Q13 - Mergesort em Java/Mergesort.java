...
class Lista {
    ...
    private void intercalar(int esquerda, int meio, int direita){
        // Dados
        int nEsq = meio - esquerda + 1;
        int nDir = direita - meio;

        // Define subarrays
        Jogador[] esquerdaArray = new Jogador[nEsq];
        Jogador[] direitaArray = new Jogador[nDir];

        // Preenche subarrays corretamente
        for (int i = 0; i < nEsq; i++) {
            esquerdaArray[i] = arrayJogadores[esquerda + i];
        }

        for (int j = 0; j < nDir; j++) {
            direitaArray[j] = arrayJogadores[meio + 1 + j];
        }

        int i = 0, j = 0, k = esquerda;

        for (; i < nEsq && j < nDir; k++) {
            int comparacao = esquerdaArray[i].getUniversidade().compareTo(direitaArray[j].getUniversidade());

            if (comparacao < 0 || (comparacao == 0 && esquerdaArray[i].getNome().compareTo(direitaArray[j].getNome()) <= 0)) {
                arrayJogadores[k] = esquerdaArray[i];
                i++;
            } else {
                arrayJogadores[k] = direitaArray[j];
                j++;
            }
        }
...
public class Mergesort {
    ...
    public static void main(String[] args) throws Exception{
        ...
        lista.mergesort(0, lista.getN() - 1);
